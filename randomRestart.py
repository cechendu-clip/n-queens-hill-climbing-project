...
def random_restart_hill_climbing(n, sw=False):
    """
    ...
    Returns: (restarts, t_steps, final, atts)
        ...
        t_steps: cumulative step count across all inner hill-climbing runs
        final: goal state found
        atts: list of (success, steps, h_seq, states) for every attempt
    """
    ...
    while True:
        ...
        t_steps += steps
        atts.append((success, steps, h_seq, states))
        if success:
            return restarts, t_steps, final, atts
        # [Slide 28: "if at first you don't succeed, try, try again"]
        restarts += 1

# Trial Runner
def _run_rr_trials(n, nt, sw):
    """
