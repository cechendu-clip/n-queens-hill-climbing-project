...
from heuristic import State, compute_heuristic, get_best_neighbors, random_state, validate_n
...
def hill_climbing(n: int, init: Optional[State] = None) -> Tuple[bool, int, State, List[int], List[State]]:
    """
    ...
    if init is not None and len(init) != n:
        raise ValueError("init lenght must equal n")
    ...
    while True:
        ...
        bh, nbrs = get_best_neighbors(current)
...
def hill_climbing_sideways(n: int, init: Optional[State] = None) -> Tuple[bool, int, State, List[int], List[State]]:
    """
    ...
    if init is not None and len(init) != n:
        raise ValueError("init length must equal n")
...
# Trial Runner
def _run_trials(n: int, nt: int, sw: bool) -> Dict[str, float]:
    """
    ...
    for _ in range(nt):
        ...
        if success:
            s_steps.append(steps)
        else:
            f_steps.append(steps)
